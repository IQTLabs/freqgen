...
program
  ...
  .option('-p, --pop-size <int>', 'the size of the population (default: 100)')
  .option('-m, --mutation-rate <int>', 'the mutation rate (default: 0.3)')
  .option(
  ...
  .action(function(options) {
    ...
    freqgen
      .generate(seq, freqs, {
        ...
        crossoverProbability: options.crossoverRate,
        geneticCode: options.geneticCode,
        maxGensSinceImprovement: options.earlyStopping,
      })
      .then(res => {
        console.log(
          `>Generated by Freqgen from ${options.seq} targeting ${options.freq}\n${res[0].entity}`
        )
      })
    // spinner.stop()
  })
